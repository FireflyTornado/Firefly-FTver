...
description: 从 NetBIOS 与 SMB 的早期时代，到 Conficker、EternalBlue、WannaCry、NotPetya，再到现代 SMB 签名、加密与 NTLM 防护：回顾 TCP 445 端口二十余年的变迁。
...
445 只是一个 TCP 端口。真正让它在网络安全史上留下浓重一笔的，是长期运行在它之上的 **SMB（Server Message Block，服务器消息块）协议**，以及围绕 SMB、Windows Server Service、RPC、NTLM 等组件发生的一系列安全事件。
...
# 一、从 NetBIOS 到 TCP 445：SMB 是如何走到这里的
...
# 二、2008：MS08-067 与 Conficker，445 开始成为“蠕虫入口”的代名词
...
# 三、2017：MS17-010、EternalBlue 与 Shadow Brokers
...
换句话说，**漏洞被发现和修复，并不等于风险已经自动消失；补丁真正部署到系统上之后，风险才会下降。**
...
也就是说，**445 是端口，SMB 是协议，MS17-010 是微软针对一组漏洞发布的安全更新，而 EternalBlue 则是针对其中 SMBv1 漏洞的利用技术之一。**
...
# 四、WannaCry 与 NotPetya：445 从此“恶名昭彰”
...
因此，WannaCry 不仅展示了 SMB 漏洞本身的危险，也成为补丁管理失败最经典的案例之一：**补丁存在，并不代表补丁已经真正进入生产环境。**
...
NotPetya 留下的教训比 WannaCry 又向前走了一步：**修复漏洞很重要，但“系统已经打补丁”并不等于整个网络已经安全。**
...
# 五、从淘汰 SMBv1 到 SMBGhost：协议升级并不等于绝对安全
...
这体现的是现代网络安全里一个非常重要的原则：

> **不要把“系统目前没有已知漏洞”等同于“这个服务适合直接暴露公网”。**

---

# 八、今天看到“有人扫描我的 445”，意味着什么？
